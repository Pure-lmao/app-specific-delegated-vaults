...
#[test]
fn app_ix_ok_expiry_zero_despite_large_clock() {
   let mut mollusk = fresh_mollusk();
   set_clock_unix(&mut mollusk, 9_999_999_999);
   let owner = Pubkey::new_unique();
   let app = test_program_id();
   let delegate = Pubkey::new_unique();
   let (pda, _) = derive_user_vault(&owner, &app);
   ...
   let r0 = mollusk.process_and_validate_instruction(&create_ix, &create_accounts, &[Check::success()]);
   log_cu("delegate_expiry::app_ix_ok_expiry_zero_despite_large_clock:create", &r0);
   ...
   log_cu("delegate_expiry::app_ix_ok_expiry_zero_despite_large_clock:deposit", &r1);

   let (treasury_pda, _) = treasury_pda();
   let treasury_ata = associated_token_address(&treasury_pda, &mint_pk);
   let owner_source = associated_token_address(&owner, &mint_pk);
   let app_ix_data = ix_app_ix(&inner_deposit_from_user(5_000));
   let vault_ix = Instruction::new_with_bytes(
      vault_program_id(),
      &app_ix_data,
      vec![
   ...
   let r_app = mollusk.process_and_validate_instruction(
      &vault_ix,
      &with_loader_accounts(&merged_app),
      &[Check::success()],
   );
   log_cu("delegate_expiry::app_ix_ok_expiry_zero_despite_large_clock:app_ix", &r_app);
}
...
#[test]
fn cpi_entry_native_ok_expiry_zero() {
   let mut mollusk = fresh_mollusk();
   set_clock_unix(&mut mollusk, T_EXPIRY as i64 + 5_000_000);
   let owner = Pubkey::new_unique();
   let app = test_program_id();
   let delegate = Pubkey::new_unique();
   let (pda, _) = derive_user_vault(&owner, &app);
   ...
   let r0 = mollusk.process_and_validate_instruction(&create_ix, &create_accounts, &[Check::success()]);
   log_cu("delegate_expiry::cpi_entry_native_ok_expiry_zero:create", &r0);
   let lamports_dest = Pubkey::new_unique();
   let native_ix = Instruction::new_with_bytes(
      test_program_id(),
      &ix_test_cpi_entry_native_via_vault(25_000),
      vec![
         AccountMeta::new_readonly(vault_program_id(), false),
         AccountMeta::new(delegate, true),
   ...
   let mut native_merged = merge_accounts(&native_user, &r0);
   ...
   let native_accounts = with_ix_sysvar_and_loaders(&native_ix, &native_merged);
   let r_native =
      mollusk.process_and_validate_instruction(&native_ix, &native_accounts, &[Check::success()]);
   log_cu("delegate_expiry::cpi_entry_native_ok_expiry_zero:native_cpi", &r_native);
}
